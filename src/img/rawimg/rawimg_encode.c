...
static struct rawimg *rawimg_decode_png(const uint8_t *src,int srcc) {
  ...
  // An extra artificial constraint: We won't accept 16 bit/channel images.
  // Downsample them to 8, to make things easy on our clients.
  // The larger samples would be encoded big-endian and everything we run on is little-endian, so it's just inconvenient.
  //XXX Rethinking pixel format descriptions again, and this might not be such a problem anymore.
  //if (pngimage->depth==16) {
  //  png_image_reformat(pngimage,8,pngimage->colortype);
  //}
